...
export function JinPageLayout({children}: {children: React.ReactNode}) {
  ...
  const sitePendingIntro = showIntro;
  ...
  return (
    <>
      ...
      <div className={sitePendingIntro ? 'site-chrome site-chrome--intro-pending' : 'site-chrome'}>
...
function CartCountButton({onOpen}: {onOpen: () => void}) {
  ...
  return (
    ...
    >
      <Await resolve={rootData?.cart}>
        {(cart) => <CartCountInner cart={(cart ?? null) as CartReturn | null} onOpen={onOpen} />}
      </Await>
    </Suspense>
  );
}
...
function CartCountInner({
  ...
  const count = activeCart?.totalQuantity ?? 0;
